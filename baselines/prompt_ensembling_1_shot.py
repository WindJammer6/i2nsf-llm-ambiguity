...
YANG_DATA_MODEL_PATH = Path(__file__).resolve().parent / "YANG_data_model.csv"
...
# Exact examples shared with schema_guided_3_shot.py.
# One intent-policy example used only by this 1-shot baseline.
...
def compose_final_policy(original_intent: str, restated_intent: str, 
  ...
  |  +--rw rules* [name]
    ...
    system_prompt = (
        ...
        "Use the supplied intent-policy example as formatting and schema-use guidance."
    ...
    user_content = (
        "One intent-policy example for reference:\n\n"
        f"Input: {example_input1}\nOutput: {example_output1}\n\n"
...
def main(policy_intent):
    ...
    yang_data = ""
    try:
        with YANG_DATA_MODEL_PATH.open("r", encoding="utf-8") as f:
            yang_data = f.read()
...
if __name__ == "__main__":
	...
	input_csv = DATASET_PATH       # CSV with 1 column: "intent"
	output_txt = PROJECT_DIR / "experiments" / "syntactic correctness" / "generated policies" / "prompt_ensembling_0_shot.txt"
